...
class Recommend: 
    ...
    def readres(self, filename): 
        ...
        while index < len(lines) - 1: 
            if "[Eval index 1] Point: " in lines[index]: 
                info = lines[index].split("Point: ")[1]
                params = eval(info)
                if not " -> Result: " in lines[index+1]: 
                    index += 1
                    continue
                info = lines[index+1].split(": ")[1]
                values = eval(info)
        ...
        varsAll = []
        for params in paramsAll: 
            vars = []
            for idx, name in enumerate(self._nameVars): 
                typename = self._typeVars[idx]
                value = params[name]
                if typename == "int": 
                    value = (int(value) - self._rangeVars[idx][0]) / (self._rangeVars[idx][1] - self._rangeVars[idx][0])
                elif typename == "float": 
                    value = (float(value) - self._rangeVars[idx][0]) / (self._rangeVars[idx][1] - self._rangeVars[idx][0])
                elif typename == "enum": 
                    value = self._rangeVars[idx].index(value) / len(self._rangeVars[idx])
                else: 
                    assert typename in ["int", "float", "enum"]
                assert not value is None
                vars.append(value)
        ...
        if len(varsAll) > self._nInit: 
            paramsAll = paramsAll[:self._nInit]
            varsAll = varsAll[:self._nInit]
            valuesAll = valuesAll[:self._nInit]

        return np.array(varsAll, dtype=np.float32), np.array(valuesAll, dtype=np.float32)
        
    def optimize(self, divide=1.0, samples=2**4, nMC=1024, batchSize=2**4, epochs=4): 
        trainX, trainY = self.readres("remorbo2_m8e16.log")
